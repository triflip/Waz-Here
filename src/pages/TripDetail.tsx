          ...
          <div className="flex items-center gap-6 mt-6">
            ...
            <button
              onClick={(e) => {
                e.stopPropagation()
                setLightbox((prev) =>
                  prev ? { ...prev, index: Math.min(lightbox.photos.length - 1, prev.index + 1) } : null
                )
              }}
              className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center text-white text-xl"
            >›</button>
          ...
          <>
            ...
            {allPhotos[activePhoto].caption && (
              <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/80 to-transparent px-4 py-3">
                <p className="text-white text-sm">{allPhotos[activePhoto].caption}</p>
              </div>
            )}
            {allPhotos.length > 1 && (
            ...
            {allPhotos.length > 1 && (
              <>
                ...
                >‹</button>
                <button
                  onClick={() => setActivePhoto((prev) => Math.min(allPhotos.length - 1, prev + 1))}
                  ...
                  <div className="flex gap-2 overflow-x-auto p-3 pb-0">
                    {stage.images.map((img, imgIndex) => (
                      ...
                      >
                        ...
                        />
                        {img.caption && (
                          <p className="text-gray-600 text-xs mt-1 w-24 truncate">
                            {img.caption}
                          </p>
                        )}
                      </div>

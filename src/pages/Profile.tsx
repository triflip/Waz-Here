...
const Profile = () => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-black flex items-center justify-center">
      <p className="text-green-500 tracking-widest animate-pulse">Loading...</p>
    </div>
  )
  ...
  const totalFlags = trips.reduce((acc, trip) => acc + (trip.stages?.length ?? 0), 0)
      ...
      <div className="flex flex-col items-center pt-12 pb-6 px-6">
        ...
        <div className="flex gap-8 mt-6">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-gray-600 text-xs mt-1">Trips</span>
          </div>

          {/* Flags */}
          <div className="flex flex-col items-center">
            <span className="text-white text-xl font-bold">{totalFlags}</span>
            <span className="text-gray-600 text-xs mt-1">Flags</span>
          </div>

          {/* Inspired — TODO: implementar quan ideas estigui fet (Dia 10) */}
          <div className="flex flex-col items-center">
            <span className="text-white text-xl font-bold">0</span>
            <span className="text-gray-600 text-xs mt-1">Inspired</span>
          </div>
        </div>

        {/* Botons — només al propi perfil */}
        {isOwnProfile && (
          ...
          <div className="flex flex-col gap-3">
            {trips.map((trip) => (
              <div
                ...
                className="flex items-center gap-4 bg-gray-900 rounded-xl overflow-hidden border border-gray-800 hover:border-green-500 transition-colors cursor-pointer p-2"
              >
                {/* Foto esquerra */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-600 text-xs mt-1">
                    {trip.stages?.length ?? 0} {trip.stages?.length === 1 ? 'flag' : 'flags'}
                  </p>
                </div>
                ...
                <button
                  onClick={(e) => e.stopPropagation()} // evita que navegui al trip

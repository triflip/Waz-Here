        ...
        <div className="flex items-center justify-around max-w-md mx-auto">
          {items.map((item) =>
            item.isMain ? (
              ...
              >
                <span className="-rotate-45 text-2xl font-black">{item.icon}</span>
              </button>
            ) : (
              <button
                key={item.path}
              ...
              >
                ...
                <span className={`text-xs transition-colors ${isActive(item.path) ? "text-primary" : "text-gray-600 group-hover:text-gray-400"}`}>
                  {item.label}
                </span>
              </button>
            )
        ...
        {items.map((item) =>
          item.isMain ? (
            <button
              key={item.path}
              onClick={() => navigate(item.path)}
              className="w-10 h-10 bg-primary hover:bg-primary/80 rounded-xl rotate-45 flex items-center justify-center text-background shadow-[0_0_7px_rgba(19,236,73,0.4)] transition-all my-2"
            >
              <span className="-rotate-45 text-lg font-black">{item.icon}</span>
            </button>
          ) : (
            <button
              key={item.path}
              ...
              className={`w-10 h-10 rounded-xl flex items-center justify-center text-xl transition-colors
                ${isActive(item.path) ? "bg-primary/10 text-primary" : "text-gray-600 hover:text-gray-400"}`}
            >
              {item.icon}
            </button>
          )
